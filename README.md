...
- 무료 코랩은 모델 베이스라인도 돌아가지 않아서(:() 코랩 프로를 결제했다.
...
- 첫번째 리더보드 대회가 너무 인프라가 좋았던 것 같다. 앞으로는 가볍지만 학습력 좋은 다양한 모델들이 연구되었으면 좋겠다~~ (like efficient model in computer vision)
- 아무나 연구해주라~~
...
- 기계탓을 하고 싶진 않지만, 어쩔 수 없이 GPU탓이다. 이건 명백하다. 코드 자체가 돌아가지 않아 결과 제출도 5번밖에 하지 못했다. 
